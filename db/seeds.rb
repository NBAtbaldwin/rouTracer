...
synonyms = File.readlines("synonyms.txt").each do |line|
  line = line.delete("/n")
end

quotes = File.readlines("workout-quotes.txt").each do |line|
  line = line.delete("/n")
end

workout_synonyms =  File.readlines("workout-synonyms.txt").each do |line|
    line = line.delete("/n")
  end

cool_synonyms = File.readlines("cool-synonyms.txt").each do |line|
  line = line.delete("/n")
end

def adjective(synonyms)
  synonyms.sample
end

def workout_quote(quotes)
  quotes.sample
end

def workout_synonym(workout_synonyms)
  workout_synonyms.sample
end

def gen_route_name(cool_synonyms)
  adj = cool_synonyms.sample
  "My #{adj} route"
end
...
def gen_activity_name(synonyms, workout_synonyms)
  adj = synonyms.sample
  workout = workout_synonyms.sample
  "#{adj} #{workout}"
end
...
def gen_comment(cool_synonyms, workout_synonyms, synonyms)
  "#{cool_synonyms.sample} #{workout_synonyms.sample}! #{synonyms.sample}."
end
...
route1 = Route.create(
  route_name: gen_route_name(cool_synonyms),
  ...
  description: workout_quote(quotes),
  est_duration: 8220,
  elevation: 364,
  distance: 7.1,
  marker_coordinates:
...
(1..300).to_a.each do |num|
  ...
  activity_hash[num] = Activity.create(
    ...
    date: dates.sample,
    title: gen_activity_name(synonyms, workout_synonyms)
  )
end
...
demo_user.activities.each do |activity|
  ...
  rand(6).times do
    Comment.create(user_id: friends.sample.id, activity_id: activity.id, body: gen_comment(cool_synonyms, workout_synonyms, synonyms))
  end
end
